...
import { submitLlamada } from '@/lib/actions/llamadas'
...
function advanceFlow(
  ...
  // Check for wildcard rule (null respuesta_valor)
  ...
  // Default: next pregunta by orden_default sequence
...
export function FlowEngine({ elector, preguntas, reglas }: FlowEngineProps) {
  ...
  const submitAnswer = useCallback(
    (valor: string) => {
      ...
      newAnswers.set(currentPreguntaId, valor)
      setAnswers(newAnswers)
      setHistory((prev) => [...prev, currentPreguntaId])
      setLocalAnswer('')

      const nextId = advanceFlow(currentPreguntaId, valor, preguntas, reglas)
      if (nextId === null) {
    ...
    [currentPreguntaId, answers, preguntas, reglas]
  )
  ...
  function goBack() {
    ...
    const newAnswers = new Map(answers)
    // Remove the answer for the previous question (will re-answer)
    // Actually we go back to prev question and remove the answer we gave when we were at prev
    // The current question's answer might not exist yet if we just advanced to it
    // We want to go back to prev and re-answer it
    setHistory(newHistory)
    setCurrentPreguntaId(prev)
    // Remove current question's answer if it exists (we're going back past it)
    if (currentPreguntaId) {
      newAnswers.delete(currentPreguntaId)
    }
    setAnswers(newAnswers)
    setLocalAnswer('')
        ...
        <div className="border rounded-lg p-6 space-y-5">
          ...
          {answers.size > 0 && (
            <div className="space-y-1 text-sm bg-muted/40 rounded p-3">
              <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide mb-2">Respuestas</p>
